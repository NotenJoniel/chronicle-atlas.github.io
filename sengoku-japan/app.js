/**
 * 日本の戦国時代 史実クロニクルタイムライン v1.0
 * 史実ベース — 応仁の乱から大坂の陣まで (1467年〜1615年)
 */
document.addEventListener('DOMContentLoaded', () => {
  // Load data from centralized JSON
  fetch(`../data/timelines/sengoku-japan.json?t=${Date.now()}`)
    .then(r => r.json())
    .then(raw => {
  const D = {
  ...
  function fmtYear(y) { return `${y}`; }
  function fmtYearJa(y) { return `${y}年`; }
  ...
  function openCharModal(charId) {
    ...
    const rl = $('cm-reading');
    if (ch.reading) { rl.textContent = ch.reading + (ch.title ? `　旧名: ${ch.title}` : ''); rl.style.display = ''; }
